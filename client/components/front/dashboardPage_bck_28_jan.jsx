...
_myImageGalleryRenderer(item) {
      ...
      <div id="postSingleImageModel" className="uk-modal coment_popup">
          ...
          <button className="uk-modal-close uk-close" type="button"></button>
            <div className="uk-grid">
              ...
              {this.loadSinglePostByInfo(this.state.clickedUser,this.state.clickedPost)}
              <h5 className="coment_heading">Comments</h5>
              <ul className="uk-comment-list">
              {this.renderComments(this.state.clickedPost)}
                  </ul>
  ...
  renderFriendList(){
    ...
    Object.keys(friends).map((key)=> {
      ...
      friendsElement.push(  <div className="uk-grid dash_top_head dash_botom_list bg_gry_lr" id={item.id}>
            ...
            <div className="uk-width-small-1-2 post_control">
              <div>
                <a href="#" className="post_txt_dashboard" data-uk-modal={post_image?"{target:'#postSingleImageModel'}":"{target:'#postSingleContentModel'}"} onClick={this.loadSinglePostContent.bind(this,item.id,user_id,postImage,item.post_content,postVideo)}>
  ...
  loadSinglePostByInfo(userId,postId){
  const{dashboardData,userAuthSession} = this.props;
    if(userId){
    if(userAuthSession.userObject.id === userId){
        var friendData = userAuthSession.userObject;
  ...
  if(postId){
    var postContent;
    var friendPosts = dashboardData.friends;
    var friendPost = friendPosts[userId];
    if(friendPost && friendPost.length > 0){
    ...
    if(latestPost){
      ...
      //text content
       ...
       return (
        ...
        <a href="#" className="post_txt_dashboard" data-uk-modal={post_image?"{target:'#postSingleImageModel'}":"{target:'#postSingleContentModel'}"} onClick={this.loadSinglePostContent.bind(this,latestPost.id,userProfile.id,postImage,latestPost.content,postVideo)}>
