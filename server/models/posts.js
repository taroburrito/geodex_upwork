...
function constructgetNewsPostsSql(userId){
  var sql = "SELECT a.*,"+
            ...
            " gx_user_details as b,"+
            " gx_users as c"+
            " WHERE a.is_news = 'yes' AND b.user_id = a.user_id"+
            " AND c.id  = a.user_id"+
            " AND a.user_id IN (SELECT receiver_id FROM `gx_friends_list` WHERE sender_id ='"+userId+"'  AND STATUS = 1 UNION SELECT sender_id  FROM `gx_friends_list` WHERE receiver_id ='"+userId+"' AND STATUS = 1)"+
            " ORDER BY a.id DESC ";
  return sql;
...
function constructGetNextPost(postid,userId){
  var query="SELECT (a.user_id) id, LOWER(first_name) first_name, LOWER(last_name) last_name, dob, gender, address, latitude, longitude, profile_image, cover_image, c.id post_id, (image) post_image, (content) post_content,youtube_url, youtube_image,is_news,news_source,title,link,(select count(1) from gx_posts where user_id = a.user_id) post_count, youtube_url, youtube_image, u.email, modified, (c.created) post_date FROM `gx_user_details` a, (SELECT * FROM gx_posts WHERE id = (SELECT MIN(id) FROM gx_posts WHERE id > "+postid+" AND user_id = "+userId+" AND content != '')) c, gx_users u WHERE a.user_id = "+userId+" AND a.user_id = c.user_id AND a.user_id = u.id GROUP BY a.user_id ORDER BY c.id DESC ";
  return query;
}
...
function constructCreatePostSqlString(formData) {
    ...
    var query = "INSERT INTO gx_posts SET " +
            ...
            ", is_news = " + mysql.escape(formData.is_news) +
              ", news_source = " + mysql.escape(formData.news_source) +
